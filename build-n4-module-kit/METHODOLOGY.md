...
- **Permissions — source vs artifact (B777 reconciled with B636, verified on a real jar 2026-09-05):** you AUTHOR grants in the source file `module-permissions.xml` (the New-Module wizard emits it and the build requires it — line 15 above); the gradle plugin INLINES its content into the jar's `META-INF/module.xml` `<permissions>…</permissions>`, so a decompiled or built module shows only the inline form (`<java-permissions type="station">` etc.) and never a separate file. Read the inline block to learn what a Tridium module grants; edit the source file to change yours; jar-signing (`NIAGARA4.RSA/SF`) is mandatory for the privileged grants. `[ev: corpus B777]`
