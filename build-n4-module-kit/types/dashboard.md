...
- **DWS1 — Write-surface: five gates (HIGH — apply to every mutating endpoint):** (1) `checkCanWrite` first line = `OPERATOR_WRITE` fail-closed (deny on no-user / no-service / `catch(Exception)`); (2) hand-rolled `X-Requested-With` guard IN the pure `route()` (framework CSRF filter covers `/rpc/*` only); (3) pin the write ORD under a `SERVICE_ORD` facade + traversal reject OR an explicit slot allowlist; (4) mutate under a per-Ord lock → HTTP **423** on contention; (5) audit every mutation (who/what/when/old→new), fire-and-forget, audit-failure never fails the write. [ev: corpus B763]
