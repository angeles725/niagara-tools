...
- **Cycle-anchor checklist — for any scheduled equipment cycle (defrost, purge, rotation), declare the anchors explicitly, in the design comment AND the code:** (1) is the DURATION timed from-actuation (the ticket arms when the output actually energizes) or from-request (armed before actuation, so any delay between request and actuation is silently counted against the duration)? (2) is the INTERVAL re-armed from-end (stamped when the cycle completes) or from-start (stamped when it begins, so a long-running cycle shortens the gap before the next one)? A `BDefrostController` that arms `durationTicket` in `beginDefrost()` (before `doDefrostFanOffExpired()` energizes the resistance) is duration-from-request; one that stamps `lastDefrostTime` in `doIntervalExpired()`/`requestDefrostCycle()` (at cycle START) is interval-from-start. Neither is automatically wrong, but an UNDECLARED anchor is: the customer's actual contract (duration-from-actuation, interval-from-end) can silently diverge from the code's anchor, and nothing surfaces the mismatch until a commissioning walkthrough compares wall-clock behavior against the spec. Stamp the persisted anchor at the CHOSEN moment (see "Anchor a free-running interval…" above for the persistence mechanics) and test restart behavior against the declared choice, not an assumed one. `[ev: retro panccadia-defrost-sequencing-hmi-reload-deltas Δ6]` `[ev: corpus BDefrostController.doIntervalExpired() ~695, beginDefrost() ~810]`
...
- **HOA precedence contract — every output with a Hand/Off/Auto slot needs an EXPLICIT precedence matrix, not an ad hoc `if` buried in the apply method:** enumerate the matrix up front — Hand/Off/Auto × every automatic mode that can also drive the same output (defrost, staged delays, stagger, sequencing, safety inhibits) — and implement the resolution as a PURE function (`resolveOutput(hoa, automaticModes...) -> command`) with JUnit covering every cell, not scattered `if(inDefrost) return;` early-outs across several methods. **Default recommendation:** manual Hand/Off dominates all automatic logic; safety inhibits (a protection latch, not a mere operational mode) are the only named exceptions, and each exception is called out explicitly in the module's doc comment and in `types/issues-and-gotchas.md` — never left implicit. See `types/logic-authoring.md` "Defrost has priority over HOA" for a documented case where an automatic mode (not strictly a safety inhibit) currently overrides HOA by design; that gap is exactly the kind an explicit matrix is meant to surface before an integrator has to work around it on the wire sheet. `[ev: retro panccadia-defrost-sequencing-hmi-reload-deltas Δ4]` `[ev: corpus BEvaporatorUnit.java ~1504]`
  - **Field-workaround-as-requirements-signal:** an operator/integrator building kitControl `Equal`/`Not`/`And`/`Or` blocks on the wire sheet to defeat a module's own precedence rule is not "misuse" to route around — it is the field TELLING you the contract is wrong for this site. Capture it as a contract change (add the missing precedence case, or an explicit enable/disable slot), not as a support ticket to close. See `types/issues-and-gotchas.md` §J1.
