...
- The SMALLEST correct module = `module.xml` (header roster + 3-part dep FLOOR) + a non-empty `module.palette` (one `<p>` per component) + a prefixed non-empty `module.lexicon` + one `B<Comp> extends BComponent` with one `Flags.SUMMARY|Flags.OPERATOR` property + one `Flags.HIDDEN` engine action + one `Clock.Ticket` armed in `started()`+`atSteadyState()` and cancelled in `stopped()` + a SIGNED jar (major-52). This skeleton is verify-gate-green AND biting-check-green by construction. `[ev: corpus B790]`
