...
version: 0.13.3                      # current kit VERSION
last_change: 2026-09-04              # Campaign 4 C4-PR1: T-group doc folds T5 (preview gate) + T6 (WARNING classify) + T8 (lexicon toFriendly)
open_issues:
  ...
  - CAMPAIGN 4 (final — exhausts the 42-lesson corpus, the T/process group): C4-PR1 (v0.13.3) folded T5 (preview-approval gate → BUILD-LOOP §3) + T6 (WARNING caught-cosmetic-vs-aborts → METHODOLOGY new Debugging section) + T8 (lexicon toFriendly consequence → METHODOLOGY reinforce); self-retro-preview-gate → folded (T4 live-anchor smoke + the pure-test recipe were already in build-verify.md, so its four lessons are all in core). REMAINING: T7 (corpus B729-B760 wiring) — C4-PR2 folds the WB ladder B751 → types/wb-widgets.md + serving/RBAC census B752/B753 → types/dashboard.md; C4-PR3 (MINOR, structural) adds a curated corpus-index nav doc under the kit + wiring. corpus-index-rt-authoring retro stays pending until its T7 body lands.
...
last_session: 2026-09-04 · Campaign 4 C4-PR1 (v0.13.3): T-group doc folds — T5 (preview gate → BUILD-LOOP §3), T6 (WARNING classify → METHODOLOGY Debugging), T8 (lexicon toFriendly → METHODOLOGY). self-retro-preview-gate → folded (T4 + pure-test recipe already in build-verify.md; avoided a T4 double-fold). next: C4-PR2 (T7 b/c doc folds wb-widgets + dashboard), then C4-PR3 (T7a corpus-index nav doc, structural, MINOR) — closes the 42-lesson corpus.
<!-- /build-state.v1 -->
