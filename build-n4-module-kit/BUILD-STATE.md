...
2026-09-20 · automation-audit PR-B: build.sh now auto-chains preflight -> gradle -> verify -> report-module in ONE invocation (--no-preflight/--no-report bypasses for inner loop); + tests/build.bats (5, none existed) + SKILL/BUILD-LOOP single-command flow. Closes the audit's 'must-remember' gap (report-module/preflight were invoked by no orchestrator). [ev: retro build-sh-auto-chain]
