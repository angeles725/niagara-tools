...
open_issues:
  ...
  - DEFERRED lint candidate `lint-set-null-ord` (from retro wb-mapping-ord-npe-and-wsl-windows-jdk Δ1, folded 2026-09-24): flag a `setXxxOrd(getSlotPathOrd())` / `set(<ordProp>, <expr that can be null>)` call with no null guard. NOT implemented — check overlap with `lint-null-context-write` first; needs its own guard-pin + bats fixtures + BUILD-LOOP/SKILL wiring pass per kit-links, not a rushed in-line addition to a doc-only fold task.
...
2026-09-20 · automation-audit PR-B: build.sh now auto-chains preflight -> gradle -> verify -> report-module in ONE invocation (--no-preflight/--no-report bypasses for inner loop); + 4 stub-driven tests in tests/build-sh.bats (BS-preflight/report) + SKILL/BUILD-LOOP single-command flow. Closes the audit's 'must-remember' gap (report-module/preflight were invoked by no orchestrator). [ev: retro build-sh-auto-chain]
2026-09-20 · lint-candidates L2: new toolbelt/lint-no-md5-credential-digest.sh (WARN, NMD2 pin) + bats + AUTO-WIRED into report-module.sh §5.21 + RM21 (so build.sh runs it) + BUILD-LOOP §5. Folds the no-md5-credential-digest candidate from wb-vendor-ux-wave3-vendor-drivers-deltas Δ13. [ev: retro lint-no-md5-credential-digest]
2026-09-20 · lint-candidates L5: new toolbelt/lint-wb-file-chooser.sh (WARN, WFC2 pin) + bats + AUTO-WIRED report-module.sh §5.22 + RM22 + BUILD-LOOP §5. Folds lint-wb-file-chooser candidate from apillm-headless-servlet-rt-4.14-deltas Δ8. [ev: retro lint-wb-file-chooser]
2026-09-20 · lint-candidates L3: new toolbelt/lint-bundled-jar-class-version.sh (FAIL module-once, BJCV2 pin) + bats + AUTO-WIRED report-module.sh §12 + RM23 + BUILD-LOOP §5. Folds bundled-jar-class-version candidate from module-hardening-failure-modes-deltas Δ11 (BLD2). Per-module lint-candidate set complete. [ev: retro lint-bundled-jar-class-version]
2026-09-20 · CI: wired lint-guard-pins --strict into .github/workflows/ci.yml (after sweep-fold-audit) — kit self-check that every toolbelt/lint-*.sh has a bats-resolvable # Mutation: guard-pin; tree clean (0 WARN). Closes the audit's 'guard-pins not gated' item. [ev: retro ci-lint-guard-pins]
2026-09-21 · lint-servlet api-response-headers: added the last unimplemented per-module lint candidate as a WARN CHECK inside lint-servlet.sh (already auto-wired report-module §5.18) — LSV7 pin, LSV7+LSV7-clean tests; + 3 stale-doc fixes (security §311/§419 no-md5, dashboard lexicon-ascii now-implemented). [ev: retro lint-servlet-api-response-headers]
2026-09-21 · apillm live-commissioning: WB manager stale-view root-caused live (BWbComponentView built once in doLoadValue, no refresh; slot values load async) → fixed BApillmManager + BApillmImporterManager with registerForComponentEvents(subject,depth)+handleComponentEvent+activated/deactivated, plus the external-ORD value/status gap (subscribe each resolved target, B1140-G1). Corpus B1140-B1142 (WB subscription mechanism + rt read-path + wb create-point). New PENDING retro apillm-wb-subscription-refresh-and-points-deltas (5Δ) + ORCHESTRATION.md cross-link to research-sdd #867. Not yet folded. [ev: retro apillm-wb-subscription-refresh-and-points-deltas]
...
2026-09-24 · fold-2026-09-21-23-retro-deltas T1 FOLDED secure-authoring-isoperational-gate (1Δ) → types/security.md new §5.1 "Ongoing-work gate — serviceStarted() only guards STARTUP" (advisory license fault; isOperational() gate on changed()/timers/servlet-write callbacks; deferred lint candidate lint-license-isoperational-gate recorded, not implemented — check overlap with lint-status-parity/lint-silent-protection first). INDEX row flipped folded, retro marker flipped. [ev: retro secure-authoring-isoperational-gate]
2026-09-24 · fold-2026-09-21-23-retro-deltas T2 FOLDED wb-mapping-ord-npe-and-wsl-windows-jdk (2Δ) → types/issues-and-gotchas.md new §H3 (Δ1 null-ORD-from-getSlotPathOrd() runtime set() NPE, distinct from the PD-FE1 picker gotcha) + new §D5 (Δ2 WSL Windows-JDK URISyntaxException symptom, points at structure.md §L10 pinning fix already in core); types/wb-widgets.md Field editors § cross-linked to §H3. Optional lint-set-null-ord candidate evaluated and DEFERRED (open_issue above), not implemented. INDEX row flipped folded, retro marker flipped. [ev: retro wb-mapping-ord-npe-and-wsl-windows-jdk]
2026-09-24 · fold-2026-09-21-23-retro-deltas T3 FOLDED live-diagnosis-hardening-deltas (6Δ, full promotion — all 3 lint asks implemented, none deferred) → new toolbelt/lint-changed-hot-write.sh (Δ1 RUN8 hot changed() fan-in + no rate guard + non-transient write, CHW2 guard-pin) + types/issues-and-gotchas.md new §I1 (Δ2); new toolbelt/lint-persist-hot-write.sh (Δ3 PER8 non-transient setter call site with no cadence guard, pairs with Δ1, PHW2 guard-pin) + §I2 (Δ4); new toolbelt/lint-session-store-lazy-evict.sh (Δ5 UXS7 static session/token Map with no sweep-on-insert or scheduled purge, SSL2 guard-pin) + §F2 (Δ6). All 3 lints validated against the real Cliente/Leon-Guanjuato-worktrees/main-ff1b659 CompPan-rt/BCompressorControl.java (pre-debounce checkout): lint-changed-hot-write and lint-persist-hot-write both correctly WARN on it (35 `p ==` branches, 3 non-transient hour-slot writes + 1 faultReset write with no cadence guard); lint-session-store-lazy-evict correctly stays clean on DashboardPan-ux/ConfigSession.java (instance-scope, not static — the already-fixed R14 shape). All 3 named in BUILD-LOOP.md (kit-links L5). tests/lint-{changed-hot-write,persist-hot-write,session-store-lazy-evict}.bats 23/23 green; each guard-pin mutation manually flipped and confirmed RED, then reverted. INDEX row flipped folded, retro marker flipped. [ev: retro live-diagnosis-hardening-deltas]
