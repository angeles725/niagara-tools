...
open_issues:
  ...
  - GATE-HARDENING open_issue (future, NOT this campaign): the §7 promotion exit requires an in-range INDEX diff, which FALSE-NEGATIVES a legitimate PARTIAL promotion (folds content but its source retro stays pending because other halves are owed — e.g. C4-PR2 flipped no row). Potential fix: let the hook accept an in-range BUILD-STATE open_issue update (narrowing the owed list) as ALTERNATIVE proof-of-promotion for a partial fold. Track as gate-hardening; do not scope-creep into another gate PR now.
