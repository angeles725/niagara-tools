...
version: 0.12.0                      # current kit VERSION
last_change: 2026-09-04              # Campaign 3 C3-PR3: implement B8 (type-count) + B10 (lightweight backup + purge) in ng-deploy.sh
open_issues:
...
last_session: 2026-09-04 · Campaign 3 C3-PR3 implemented B8 (type-count fix) + B10 (lightweight backup + keep-N purge + --no-backup WARN + --full-backup) in ng-deploy.sh → folds ng-deploy-type-count-and-cwd + ng-deploy-backup-liviano-y-autopurga. next: C3-PR4 (verify-module palette check → folds module-palette), then the final tidy pass (3 items above)
<!-- /build-state.v1 -->
