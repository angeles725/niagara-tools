...
for ADIR in "${ARTIFACTS[@]}"; do
  ...
  # ----------------------------------------------------------------
  # 5.22. lint-wb-file-chooser.sh (WARN; only *-wb artifacts with src/)
  #        A BWbFieldEditor.dialog(...BOrd.NULL) without BComponentChooser/targetType
  #        opens the file-system chooser (C:\), useless in a station context.
  # ----------------------------------------------------------------
  case "$ANAME" in
    *-wb)
      if [ -d "$ADIR/src" ]; then
        wfc_exit=0
        wfc_out=$("$TOOLBELT/lint-wb-file-chooser.sh" "$ADIR/src" 2>&1) || wfc_exit=$?
        if [ "$wfc_exit" -eq 3 ]; then
          emit "$ANAME" ERROR lint-wb-file-chooser "env fault (exit 3)"; HAD_ENV=1
        else
          while IFS= read -r _ln; do
            [ -z "$_ln" ] && continue
            case "$_ln" in
        ...
        emit "$ANAME" SKIP lint-wb-file-chooser "no src/"
      fi
    ;;
  esac
