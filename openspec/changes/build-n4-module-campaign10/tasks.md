...
- [ ] 1.10 OBSERVED mutation: "drop `brace_depth >= 2` guard from the section-D port in lint-timers.sh → anyNoHardware re-FAILs (S21-neg flips back to FAIL)"; record verbatim RED-then-GREEN. `[ev: SC-7; D1b]`
...
**Rationale**: the cwd-sensitive part is the RUNTIME test read — `WiringTests` read `Paths.get("src/…")` and `../../build.gradle.kts` relative to JVM cwd. `-sourcepath` is NOT broken (coordinator validator §Claim 3). Fix is runner-side only, TWO edits. No client test file touched (K12/K13). `[ev: apply-package S24 4d5e6092c §Claim 3]`
...
- [ ] 4.4 Edit 1 — `build-n4-module-kit/toolbelt/run-pure-test.sh`: after the `[ -d "$rt" ]` guard at `:30`, add `rt=$(cd "$rt" && pwd)` — normalise `$rt` to an absolute path so a relative `$1 = .` (regression case) survives any later `cd`. This is a defensive absolutise; `-sourcepath "$rt/src:$testroot"` at `:58-60` is not broken. `[ev: D4b; kit run-pure-test.sh:30 @ f90b8d1]`
- [ ] 4.5 Edit 2 — wrap the `java` call at `:62` in a subshell with `cd "$rt"`: `( cd "$rt" && java -cp "$tmp:$JU:$HC" org.junit.runner.JUnitCore "$testfqcn" )`. `$tmp`/`$JU`/`$HC` are already absolute after edit 1; the subshell keeps parent cwd and `trap 'rm -rf "$tmp"' EXIT`; JUnit exit propagates via `set -euo pipefail`. This is the fix for the runtime cwd-sensitivity — NOT a claim about -sourcepath. `[ev: D4a; kit run-pure-test.sh:62 @ f90b8d1; coordinator validator §Claim 3]`
- [ ] 4.6 Run `bats build-n4-module-kit/tests/` GREEN from kit root, from the profile dir, and from `/tmp` (same verdict from three cwds); S24-cwd-regression still passes; no existing test's verdict changes. `[ev: SC-4]`
- [ ] 4.7 OBSERVED mutations: "revert subshell `cd "$rt"` at `:62` → S24-cwd FAIL (WiringTests cannot find src from non-profile cwd)"; "revert `rt=$(cd "$rt" && pwd)` → S24-cwd-regression FAIL when `$1 = .`." Record verbatim RED-then-GREEN. `[ev: SC-7; D4c]`
- [ ] 4.8 Verify no file under any client worktree was written (K12; K13 — only runner touched). `[ev: K12; K13]`
- [ ] 4.9 `shellcheck 0.10.0` exit 0; `bats tests/` all green; no K19 routing row change needed (run-pure-test.sh is a runner, not a lint; no BUILD-LOOP.md entry — confirm existing routing rows untouched). Retro, fragment-merge always-conflict files, commit (no trailer — K11), push, rebase before QA ping. `[ev: SC-9; SC-10; K11]`
- [ ] **[lead]** S24-cwd passes from 3 cwds · S24-cwd-regression passes · 2 OBSERVED mutation flips · no client test file in diff · `shellcheck` 0 · 0 trailers · ledger settle.
