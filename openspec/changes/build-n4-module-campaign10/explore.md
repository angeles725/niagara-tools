...
1. **S22 contract change — EW10 CompPan-rt 0→1.** The re-cut lives in `qa/c10-ext-writable-per-slot` 00e31ae; a worker that reads the C9 tip (3726722) ships the wrong contract. K13: the RED is the contract; exact-count pin = 1 WARN, subject BCompressorControl.java:1612 faultReset, no other CompPan-rt slot flips. `[ev: QA 2026-09-07 RED note]`
2. **B831-G1 (doAckAlarm indirection) can turn the S22 fix into a fresh FP.** An action `ackAlarm` whose write lives in `doAckAlarm` must be followed into the `do`-body; otherwise the rule exempts nothing and WARNs a slot that IS action-written. QA asked to confirm fixture coverage. `[ev: B831-G1]`
