...
| **R2** | **S22** `lint-ext-writable-shape.sh` per-slot writing action — replace class-level `has_action` (`:82-91`) with a per-slot body-follow: a complex OPERATOR slot `X` is exempt only when an `@NiagaraAction(name="x")` maps to its `doX()` body and THAT body writes `X` (`setX(`, `set<X>(`, `.set(<Xprop>,`), under a scope filter excluding `execute()`/`changed()`/generated setters. Built as a **NEW action-aware pass IN-SCRIPT**, over the section-D method-boundary parser (`:250-306`) + the Pass-0 paren-join — **NOT** a reuse of `lint-silent-protection.sh`, which never reads `@NiagaraAction` at all. **CONTRACT CHANGE — EW10 CompPan-rt 0 → 1** | kit | The only false-NEGATIVE in the wave: today the rule reports clean on a real defect. `[ev: apply-package S22 2f710626d]` `[ev: B831 §S22]` `[ev: investigador1 685e7981e]` |
...
| **R5** | **S25** `lint-write-path.sh` STALE advisory + `--strict` — a NEW advisory class `STALE` (a matrix row whose slot name matches no `@NiagaraProperty` / `--bog` slot in source), row grammar `STALE  lint-write-path  <matrix-path>:<line>  slot <name>: no source slot with that name` (STATUS-first, the shape of the existing FAIL row at `:374`); default exit **0** (advisory), `--strict` promotes STALE to exit **1**; the existing uncovered **FAIL exit 1 is untouched** (never weakened to WARN-only), exit 3 unchanged | kit | Verified at `cb79676`: the lint is already a hard FAIL gate; the draft's "add --strict" premise was wrong. Weakening a shipped gate to WARN-only was REJECTED (a worker forgetting `--strict` would ship an uncovered OPERATOR slot). K20 disjoint exit codes preserved. `[ev: QA 2026-09-07 S25 question; lead decision]` |
...
- `module-ext-writable-shape-lint`: exemption narrows from class-level "any `@NiagaraAction`" to **per-slot**: the action's `do<Action>()` body must write **that** slot; an action whose `do`-body writes a DIFFERENT slot does not exempt. **Contract change** — EW10 re-pins CompPan-rt from 0 to **1 WARN** (subject `faultReset`). EW3 `setSetpoint` stays clean; EW6 stays WARN; the C9 EW-token / child-leaf note is unchanged. `[ev: B831 §S22]` `[ev: RED EW-s22-neg2 @ 954ebd7]`
