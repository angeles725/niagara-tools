...
| **R4** | **S24** cwd-independent structural REDs — `run-pure-test.sh` / structural test classes resolve `src` from the **test's own location**, never from `cwd`. Lands BEFORE any further C10 structural RED is authored | kit/test | Every structural RED authored after this inherits the fix; every one authored before it inherits the bug. `[ev: explore-draft 45d550dac §S24]` |
...
| `build-n4-module-kit/toolbelt/run-pure-test.sh` | Modified | R4 — enter the module dir before the `java` call (`:62`) |
| `build-n4-module-kit/toolbelt/run-pure-test.sh` + structural test classes | Modified | R4 — resolve `src` from the test's own location |
...
- [ ] **SC-4 (S24)** — the structural suite yields the **same verdict from three different cwds** (repo root, profile dir, absolute invocation from `/`); re-introducing the cwd assumption fails the pin (OBSERVED flip); no existing test's verdict changes.
...
- `sdd-design` (parallel with `sdd-spec`) — the awk class-scope field-collector pass and the backward method-body anchor; reuse of `lint-silent-protection.sh:124-165` `SURF_WRITE` for the S22 follow; the adapter→pure resolution algorithm and its convention limits; fixture layout with comment decoys; the test-location-relative source resolver; the 7-PR chain and fragment-merge mechanics.
