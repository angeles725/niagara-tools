...
- [x] 11.2 Write `toolbelt/lint-wb-threading.sh <wb-src-dir>`: WARN `doInvoke` bodies (expanded via same-class private/protected callees to depth 3, cycle-safe) calling `getNavChildren|getNavNodes|BqlQuery` without `invokeLater|BJobService|JobThread` on the chain (B809 heuristic — human review, not FAIL); WARN `@AgentOn(baja:Component)` with no justification comment within 3 lines above; `--strict` promotes WARNs to exit 1; exits 0/1/3; `shellcheck` clean; dot-dirs pruned (D9b). WBT1d pin added: doInvoke → helperA → helperB → getNavChildren (3-level chain WARNs); WBT1d-guard: helperB via invokeLater does NOT WARN.
...
- [x] 11.8 **Named mutations** (5 total, all proven): (a) drop `check_wb_scaffold` → wb-scaffold row absent; (b) drop `_AWK_THREAD` → ui-thread-traversal absent, exit 0; (c) drop `has_guard` exemption → GuardedTraversal.java WARN; (d) drop dot-dir prune → `.deploy-baseline/Stale.java` WARN; (e) revert depth-3 expansion → DeepTraversal.java not flagged. See retro evidence table.
- [x] 11.9 Real smoke: `lint-wb-threading.sh` on chihuahua-wb BBatchLinkEditor.java → `ui-thread-traversal WARN :303 doInvoke -> searchNavTree() -> collectNavMatches() -> getNavChildren` + `agent-breadth WARN :67`; `slot-coverage.sh` on chihuahua-wb → `FAIL missing lexicon with 1 declared type(s)` exit 1. See retro Evidence §smokes.
- [x] 11.10 Guards: 256/256 bats; `shellcheck` exit 0 on all 3 scripts; `sweep-build-state.sh` exit 0; `sweep-fold-audit.sh --strict` exit 0 (56 folded, 56 cited); `kit-links.bats` 6/6 green (L4/L5 confirmed).
