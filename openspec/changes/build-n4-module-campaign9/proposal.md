...
| **R14** | **S12-B2** in-module config login for the HMI panel — SPA modal → `POST /dashboardpan/config/login` re-authenticates a station user (the kiosk login stays shared), server-held config session (TTL, sliding expiry) + `/config/logout`; every write passes `DashboardWriteGuards.evaluate` with that session and runs `parent.set(prop, toSet, cx)` with the re-authenticated user's Context so AuditHistory and the mirrored `change_log` row attribute the REAL operator (`config_session` = station username) | client (DashboardPan-ux) | User decision 2026-09-06: shared kiosk login + a second identifying login before changes; no credential storage in the module (retracted); legal Niagara re-auth API pending research block B830 (`[INFER]` until then) |
...
- `module-demand-scope-lint`: `lint-demand-scope.sh` contract — a staged process must decide WHETHER to run (a zero-demand idle branch) before HOW; WARN (advisory, never a hard FAIL — B820 §820.3: a pure modulator fed by an upstream demand gate would false-positive) a `step`/staging body that computes a target with no zero-demand short-circuit; row `WARN  demand-in-scope  <file>:<line>  <reason>`; exits 0 (WARN only) / 1 only under `--strict` / 3 no sources or usage (K20 disjoint); CLI `[--strict] <java-src-dir>` as the RED `2916954` pins (module-root profile discovery is a later additive flag, not pinned). `[ev: corpus B820]`
...
- `write-config-step-up` *(tunnel)*: `POST /config/login {password}` checks ONE shared config password (`cfg.CONFIG_PASSWORD` in config.env — user decision 2026-09-06: identity = the viewer's JWT email, already audited; the shared secret is the authorization to configure; per-user Supabase re-auth and a configurator role list are C10 seeds) and mints a server-held short-TTL token bound to `(email + purpose="config-write")` with sliding inactivity expiry, transported as the `x-config-token` header; `/config/logout` revokes immediately; `/write` and `/alarms/ack` gate on it; read endpoints do not. NO JSON user+password store. `[ev: corpus S12]` `[ev: corpus B803 §803.6]`
...
| `config_session` on surface B | station username from the in-module config login (R14); NULL only for writes made before R14 ships | Surface-B step-up (S12 plan Part 3 — re-auth modal + real `x-niagara-csrfToken`) is NOT in C9. Surface B's identity comes from the Niagara request user via the real Context; the session column stays empty rather than being faked. |
...
| **K22** (ALREADY PRESENT at `METHODOLOGY.md:86`, folded by the C8 close — R12 verifies it with an idempotent guard, never duplicates) — a real-tree smoke on every client module root is part of the lead gate, and a smoke pin asserts exact counts AND subjects AND absence; a skipped smoke BLOCKS, never downgrades to advisory | `METHODOLOGY.md` §Kit maintenance | `[ev: retro campaign8-close-process-meta-lessons lesson 11a/11b]` |
