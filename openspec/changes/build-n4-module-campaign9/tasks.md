...
- [ ] 1.1 Re-read RED tip `cf28572` (K13): confirm compile symbols `Cfg.rotationIntervalMs` (long), `Cfg.rotationMode` (int), `ROTATION_MAKE_BEFORE_BREAK` (static final int), `ctl.swaps` (int, package-visible); confirm ROT1/ROT4 include N=3 case (D1b, D1c); confirm `pickLeastHoursOffAuto` skip `modes[k] != MODE_AUTO`.
- [ ] 1.2 Cherry-pick / merge RED branch into `feat/c9-comppan-rotation` as commit 1 (suite is RED before impl).
- [ ] 1.3 Add `BRotationMode.java`: new frozen enum `{makeBefore(0), breakBefore(1)}`; ordinals match `ROTATION_MAKE_BEFORE_BREAK = 0` / `ROTATION_BREAK_BEFORE_MAKE = 1` (D1b).
- [ ] 1.4 Add two `@NiagaraProperty` slots to `BCompressorControl.java`: `rotationInterval` (`BRelTime`, `SUMMARY|OPERATOR`, min 0, max 24h) and `rotationMode` (`BRotationMode`, `SUMMARY|OPERATOR`); shape precedent: `powerOnDelay` at `:383-388`; MIN and MAX both supplied (D1).
- [ ] 1.5 Add adapter wiring beside the `minOnMs/minOffMs/stageDelayMs` trio at `:1907-1909`: `cfg.rotationIntervalMs = getRotationInterval().getMillis();` and `cfg.rotationMode = getRotationMode().getOrdinal();` (D1).
- [ ] 1.6 Optionally surface `setRotationSwaps(ctl.swaps)` as a READONLY slot; if added, include its write-path matrix row (D1).
- [ ] 1.7 In `CompressorControl.java`: add transient fields `rotOut = -1`, `rotArmedMs`, `rotSinceMs[]`, `swaps`; mode constants `ROTATION_MAKE_BEFORE_BREAK = 0`, `ROTATION_BREAK_BEFORE_MAKE = 1` (D1b, D1d).
- [ ] 1.8 Clear `rotOut`, `rotArmedMs`, `rotSinceMs[]`, `swaps` in `resetTransient()` at `:328-335`; re-seed `rotSinceMs[]` alongside `cmdSince[]` in `seedRestart(now)` at `:346-349` (D1d).
- [ ] 1.9 Stamp `rotSinceMs[k] = now` in the ordinary stage-up write at `:229` (stamped whenever a unit is commanded ON by staging or by rotation arm) (D1).
- [ ] 1.10 Add `pickLeastHoursOffAuto(now, minOffMs)`: skip condition `cmd[k] || modes[k] != MODE_AUTO`; do NOT modify existing `pickLeastHoursOff` (ROT5 golden requires the stage-up path unchanged) (D1).
- [ ] 1.11 Insert **step 2b** (rotation COMPLETION) after `:217` clamp-end, before `:219` step-3 comment: when `rotOut >= 0 && (now - rotArmedMs) >= stageDelayMs`, execute completion — `cmd[rotOut] = false; cmdSince[rotOut] = now; lastStageMs = now; rotOut = -1; swaps++` (make-before-break default); handle edge cases ROT11-ROT15 per D1c (demand-rise cancel, demand-fall drop-rotOut-first, dischargeHigh mid-window, HAND/OFF flip mid-window).
- [ ] 1.12 Insert **step 3b** (rotation ARM) after `:246` stage-move close, before `:248` step-4 comment: evaluate 10 gates in order (D1 gate table, gates 1-10); arm action: `cmd[in] = true; cmdSince[in] = rotSinceMs[in] = now; lastStageMs = now; rotOut = out; rotArmedMs = now` (D1).
- [ ] 1.13 Handle `rotationMode = ROTATION_BREAK_BEFORE_MAKE`: invert completion order (drop `rotOut` first, add `in` after `stageDelay`).
- [ ] 1.14 Bump `Compresores/build.gradle.kts:33` from `2.0.3` to **`2.1.0`** — fragment-merge constraint: PR9 bumps this same line to 2.2.0 later; PR1 sets ONLY 2.1.0.
- [ ] 1.15 Add two write-path matrix rows in `docs/write-path-matrix.md` for `rotationInterval` and `rotationMode` (D11c decision: PR1 owns its 2 S20 rows; PR11 owns the remaining 62).
- [ ] 1.16 **Named mutations** (all OBSERVED): (a) remove `!= MODE_AUTO` from `pickLeastHoursOffAuto` → ROT7 HAND-in-minOff fixture flips GREEN→RED; (b) delete completion `lastStageMs = now` → ROT4/ROT11 fail on N=3 fixture; (c) base clock on `cmdSince[k]` instead of `rotSinceMs[k]` → ROT16 fails on enable-without-full-interval fixture. Record verbatim output for each.
- [ ] 1.17 `schema-risk.sh` SAFE on real `config.bog` snapshot (R1.11).
- [ ] 1.18 Real-tree smoke (CD10): `lint-demand-scope.sh`, `lint-silent-protection.sh`, `lint-ext-writable-shape.sh` each on ColdRoomPan-rt · CompPan-rt · DashboardPan-rt · DashboardPan-ux — exact count + subject + absence of false positives; dot-dirs not traversed; `schema-risk.sh` = SAFE for CompPan-rt.
- [ ] 1.19 Guards: `javac`/`slotomatic` exits 0; `sweep-build-state.sh`; 0 attribution trailers in commit bodies (K11/CD7); rebase `feat/c9-comppan-rotation` onto client main before QA ping (CD8); verify `git log -1` = blessed tip before settle.
- [ ] **[lead]** ROT1-ROT16 + golden green; OBSERVED flips for all 3 named mutations; `schema-risk.sh` SAFE; `vendorVersion = 2.1.0` confirmed in `CompPan-rt/module.xml`; merge ff-only; ledger acquire + settle `--max-changed-lines 300 --declare-exception`.
