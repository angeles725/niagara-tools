...
A real-tree smoke MUST assert the **current clean state** of the tree, not a known bug. A smoke that pins a FAIL because a known defect is present on the target tree (not because the lint rule fires on a synthetic fixture) is pinning a bug, not a rule. When the bug is fixed the smoke either becomes vacuously green (wrong reason) or red (rot). The rule is pinned by synthetic fixtures; the real-tree smoke pins that the tree is currently clean. A smoke that must assert a FAIL requires a named issue ticket alongside the pin. `[ev: proposal §Intent ¶4; proposal SC-7; lead re-measure 2026-09-07 on main-ff1b659]`
